...
// 1. Create BulkSmsTool.tsx
const bulkSmsCode = `import React, { useState, useEffect } from 'react';
...
`
fs.writeFileSync('src/components/tools/BulkSmsTool.tsx', bulkSmsCode);

// 2. Patch App.tsx
let appCode = fs.readFileSync('src/App.tsx', 'utf8');
const lazyImport = "const BulkSmsTool = lazy(() => import('./components/tools/BulkSmsTool').then(module => ({ default: module.BulkSmsTool })));";
appCode = appCode.replace("const DailyKhata = lazy(() => import('./components/tools/DailyKhata').then(module => ({ default: module.DailyKhata })));", lazyImport + "\nconst DailyKhata = lazy(() => import('./components/tools/DailyKhata').then(module => ({ default: module.DailyKhata })));");
...
`;
appCode = appCode.replace("{/* VIEW: DAILY KHATA */}", viewBlock + "\n            {/* VIEW: DAILY KHATA */}");
...
`;
sidebarCode = sidebarCode.replace("id: 'daily_khata',", sidebarItem + "\n    {\n      id: 'daily_khata',");
fs.writeFileSync('src/components/layout/Sidebar.tsx', sidebarCode);

// 4. Patch ToolsHub.tsx
let toolsHubCode = fs.readFileSync('src/components/tools/ToolsHub.tsx', 'utf8');
const toolEntry = `    { id: 'bulk-sms', category: 'utility', nameHi: 'SMS / WhatsApp अलर्ट', nameEn: 'Bulk SMS Alerts', descHi: 'ग्राहकों को एक साथ मैसेज भेजें।', descEn: 'Send bulk status updates to customers.', target: 'bulk_sms', subCategory: 'utility' },`;
toolsHubCode = toolsHubCode.replace("    { id: 'cibil-checker',", toolEntry + "\n    { id: 'cibil-checker',");
