...
// The problematic lines likely look like:
// recipientHi: 'सेवा में,
// श्रीमान...
// '
// Let's replace the single quotes with backticks around these strings.

// I will just re-apply the custom template block correctly.
// First, find and remove the old broken block.
code = code.replace(/\{\s*id: 'tpl-custom-general',[\s\S]*?\},/m, "");
...
code = code.replace("export const INITIAL_APPLICATION_TEMPLATES: ApplicationTemplate[] = [", "export const INITIAL_APPLICATION_TEMPLATES: ApplicationTemplate[] = [\n" + customTemplate);

fs.writeFileSync('src/data/initialTemplates.ts', code);
console.log("Fixed custom template string literals in initialTemplates.ts");
